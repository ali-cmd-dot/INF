...
export default function ScoreCard({ company, rank, isOther }) {
  ...
  const s = isOther
    ? { color: "#475569", border: "rgba(71,85,105,0.2)", bg: "rgba(71,85,105,0.07)", label: "N/A" }
    : getStyle(avgScore);

  const pct     = Math.min(avgScore, 100);
  const CARD_BG = "#0C1525";
  const BDR     = "#1A3050";

  const shortName = company.company.length > 24
  ...
  return (
    <>
      <div
        ...
        style={{
          background: CARD_BG,
          border: isOther ? `1px dashed ${BDR}` : `1px solid ${s.border}`,
        ...
        onMouseEnter={(e) => {
          ...
          e.currentTarget.style.boxShadow = isOther
            ...
            : `0 16px 48px rgba(0,0,0,0.5), 0 0 0 1px ${s.color}18`;
        }}
        onMouseLeave={(e) => {
      ...
      >
        ...
        {rank && !isOther && (
          <div style={{
            ...
            background: rank <= 3 ? "rgba(245,158,11,0.12)" : "rgba(26,48,80,0.5)",
            border:      rank <= 3 ? "1px solid rgba(245,158,11,0.45)" : `1px solid ${BDR}`,
            display: "flex", alignItems: "center", justifyContent: "center",
            fontFamily: "'Space Grotesk',sans-serif", fontSize: 12, fontWeight: 700,
            color: rank <= 3 ? "#F59E0B" : "#2D4A6A",
          }}>
            #{rank}
          </div>
        )}
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 22, paddingRight: 42 }}>
          ...
          <div style={{ flex: 1, minWidth: 0, paddingTop: 3 }}>
            <div title={company.company} style={{
              fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 15,
              color: isOther ? "#475569" : "#E8EEF6", lineHeight: 1.3, marginBottom: 5,
              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
            }}>
              {shortName}
            </div>
            <div style={{ fontSize: 12, color: "#2D4A6A" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 14 }}>
            <div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 3 }}>
                <span style={{
                  ...
                  fontSize: 48, lineHeight: 1, letterSpacing: "-0.04em", color: s.color,
                }}>
                  {avgScore.toFixed(1)}
                </span>
                <span style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 500, fontSize: 20, color: "#2D4A6A", lineHeight: 1.5 }}>
                  /100
                </span>
              </div>
            ...
            {!isOther && (
              <div style={{
                ...
                fontSize: 11, fontWeight: 700, color: s.color, letterSpacing: "0.05em",
              }}>
                {s.label}
              </div>
            )}
          ...
          <div style={{ height: 5, background: "rgba(26,48,80,0.6)", borderRadius: 99, overflow: "hidden" }}>
            <div style={{
              ...
              background: isOther ? "#1A3050" : `linear-gradient(90deg,${s.color}60,${s.color})`,
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 16, borderTop: "1px solid rgba(26,48,80,0.6)" }}>
          <span style={{ fontSize: 13, color: "#2D4A6A" }}>
            Total:{" "}
            <span style={{ fontFamily: "'Space Grotesk',sans-serif", color: "#3D5A7A", fontWeight: 600 }}>
